...
def create_default_environment():
    ...
    def native_print(arguments: list[inter.RuntimeValue]):
        ...
        print(*args)
    env.declare("print", inter.NativeFunction(native_print), True, None)

    def native_time(arguments: list[inter.RuntimeValue]):
        return inter.NumberValue(time.time())
    env.declare("time", inter.NativeFunction(native_time), True, None)

    def native_sin(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.sin(x)
        return inter.NumberValue(y)
    env.declare("sin", inter.NativeFunction(native_sin), True, None)

    def native_cos(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.cos(x)
        return inter.NumberValue(y)
    env.declare("cos", inter.NativeFunction(native_cos), True, None)

    def native_tan(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.tan(x)
        return inter.NumberValue(y)
    env.declare("tan", inter.NativeFunction(native_tan), True, None)

    def native_asin(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.asin(x)
        return inter.NumberValue(y)
    env.declare("asin", inter.NativeFunction(native_asin), True, None)

    def native_acos(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.acos(x)
        return inter.NumberValue(y)
    env.declare("acos", inter.NativeFunction(native_acos), True, None)

    def native_atan(arguments: list[inter.RuntimeValue]):
        x = arguments[0].value
        y = math.atan(x)
        return inter.NumberValue(y)
    env.declare("atan", inter.NativeFunction(native_atan), True, None)
    ...
    def native_atan2(arguments: list[inter.RuntimeValue]):
        ...
        y = math.atan2(x1, x2)
        return inter.NumberValue(y)
    env.declare("atan2", inter.NativeFunction(native_atan2), True, None)
    ...
    env.declare("__rand_value__", inter.NativeFunction(native_rand_seed), True, None)

    def native_round(arguments: list[inter.RuntimeValue]):
        input = arguments[0].value
        output = round(input)
        return inter.NumberValue(output)
    env.declare("round", inter.NativeFunction(native_round), True, None)

    def native_isnan(arguments: list[inter.RuntimeValue]):
        input = arguments[0].value
        output = math.isnan(input)
        return inter.NumberValue(output)
    env.declare("isnan", inter.NativeFunction(native_isnan), True, None)

    def native_isinf(arguments: list[inter.RuntimeValue]):
        input = arguments[0].value
        output = math.isinf(input)
        return inter.NumberValue(output)
    env.declare("isinf", inter.NativeFunction(native_isinf), True, None)

    def native_sleep(arguments: list[inter.RuntimeValue]):
        milliseconds = arguments[0].value
        time.sleep(milliseconds / 1000)
        return inter.NoneValue()
    env.declare("sleep", inter.NativeFunction(native_sleep), True, None)

    def native_input(_arguments: list[inter.RuntimeValue]):
        return inter.StringValue(input())
    env.declare("input", inter.NativeFunction(native_input), True, None)

    env.declare("None", inter.NoneValue(), True, None)
    env.declare("inf", inter.NumberValue(float("inf")), True, None)
    env.declare("nan", inter.NumberValue(float("nan")), True, None)
